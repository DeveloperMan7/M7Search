...
#import "M7SearchSuggestionViewController.h"

#define M7RectangleTagMaxCol 3
#define M7TextColor M7SEARCH_COLOR(113, 113, 113)
#define M7SEARCH_COLORPolRandomColor self.colorPol[arc4random_uniform((uint32_t)self.colorPol.count)]

@interface M7SearchViewController () <UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource, M7SearchSuggestionViewDataSource, UIGestureRecognizerDelegate> {
 ...
 */
@property (nonatomic, weak) M7SearchSuggestionViewController *searchSuggestionVC;
...
@implementation M7SearchViewController
...
- (void)viewDidLayoutSubviews
{
    ...
    if (self.searchViewControllerShowMode == M7SearchViewControllerShowModePush) {
        UIButton *backButton = self.navigationItem.leftBarButtonItem.customView;
    ...
    // More details : https://github.com/iphone5solo/M7Search/issues/108
    if (@available(iOS 11.0, *)) { // iOS 11
        if (self.searchViewControllerShowMode == M7SearchViewControllerShowModeModal) {
            NSLayoutConstraint *leftLayoutConstraint = [searchBar.leftAnchor constraintEqualToAnchor:titleView.leftAnchor];
            if (navigationBarLayoutMargins.left > M7SEARCH_MARGIN) {
                [leftLayoutConstraint setConstant:0];
            } else {
                [leftLayoutConstraint setConstant:M7SEARCH_MARGIN - navigationBarLayoutMargins.left];
            }
        }
        searchBar.py_height = self.view.py_width > self.view.py_height ? 24 : 30;
        searchBar.py_width = self.view.py_width - adaptWidth - M7SEARCH_MARGIN;
        ...
        if (self.searchViewControllerShowMode == M7SearchViewControllerShowModePush) {
            titleView.py_width = self.view.py_width - adaptWidth - M7SEARCH_MARGIN;
        } else {
            titleView.py_x = M7SEARCH_MARGIN * 1.5;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (NO == self.navigationController.navigationBar.translucent) {
        ...
        if (!self.navigationController.navigationBar.barTintColor) {
            self.navigationController.navigationBar.barTintColor = M7SEARCH_COLOR(249, 249, 249);
        }
    }
    ...
    if (_searchViewControllerShowMode == M7SearchViewControllerShowModePush) {
...
{
    M7SearchViewController *searchVC = [[self alloc] init];
    searchVC.hotSearches = hotSearches;
    searchVC.searchBar.placeholder = placeholder;
    return searchVC;
}

+ (instancetype)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder didSearchBlock:(M7DidSearchBlock)block
{
    M7SearchViewController *searchVC = [self searchViewControllerWithHotSearches:hotSearches searchBarPlaceholder:placeholder];
...
- (M7SearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        M7SearchSuggestionViewController *searchSuggestionVC = [[M7SearchSuggestionViewController alloc] initWithStyle:UITableViewStyleGrouped];
...
#pragma clang diagnostic pop
        ...
        searchSuggestionVC.view.frame = CGRectMake(0, CGRectGetMaxY(self.navigationController.navigationBar.frame), M7ScreenW, M7ScreenH);
...
- (UIButton *)emptyButton
{
    if (!_emptyButton) {
        ...
        [emptyButton setTitleColor:M7TextColor forState:UIControlStateNormal];
        [emptyButton setTitle:[NSBundle py_localizedStringForKey:M7SearchEmptyButtonText] forState:UIControlStateNormal];
        ...
        emptyButton.py_width += M7SEARCH_MARGIN;
        emptyButton.py_height += M7SEARCH_MARGIN;
...
- (UIView *)searchHistoryTagsContentView
{
    if (!_searchHistoryTagsContentView) {
        ...
        searchHistoryTagsContentView.py_y = CGRectGetMaxY(self.hotSearchTagsContentView.frame) + M7SEARCH_MARGIN;
...
- (UILabel *)searchHistoryHeader
{
    if (!_searchHistoryHeader) {
        UILabel *titleLabel = [self setupTitleLabel:[NSBundle py_localizedStringForKey:M7SearchSearchHistoryText]];
...
- (void)setup
{
    ...
    [cancleButton setTitle:[NSBundle py_localizedStringForKey:M7SearchCancelButtonText] forState:UIControlStateNormal];
    ...
    cancleButton.py_width += M7SEARCH_MARGIN;
    ...
    [backButton setTitle:[NSBundle py_localizedStringForKey:M7SearchBackButtonText] forState:UIControlStateNormal];
     ...
     * Initialize settings
     */
    self.hotSearchStyle = M7HotSearchStyleDefault;
    self.searchHistoryStyle = M7HotSearchStyleDefault;
    self.searchResultShowMode = M7SearchResultShowModeDefault;
    self.searchViewControllerShowMode = M7SearchViewControllerShowDefault;
    self.searchSuggestionHidden = NO;
    self.searchHistoriesCachePath = M7SEARCH_SEARCH_HISTORY_CACHE_PATH;
    ...
    headerView.py_width = M7ScreenW;
    headerView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    UIView *hotSearchView = [[UIView alloc] init];
    hotSearchView.py_x = M7SEARCH_MARGIN * 1.5;
    ...
    UILabel *titleLabel = [self setupTitleLabel:[NSBundle py_localizedStringForKey:M7SearchHotSearchText]];
    ...
    hotSearchTagsContentView.py_y = CGRectGetMaxY(titleLabel.frame) + M7SEARCH_MARGIN;
    ...
    emptySearchHistoryLabel.text = [NSBundle py_localizedStringForKey:M7SearchEmptySearchHistoryText];
...
- (UILabel *)setupTitleLabel:(NSString *)title
{
    ...
    titleLabel.tag = 1;
    titleLabel.textColor = M7TextColor;
    [titleLabel sizeToFit];
    titleLabel.py_x = 0;
    titleLabel.py_y = 0;
    return titleLabel;
...
- (void)setupHotSearchRectangleTags
{
    ...
    contentView.py_width = M7SEARCH_REALY_SCREEN_WIDTH;
    contentView.py_x = -M7SEARCH_MARGIN * 1.5;
    contentView.py_y += 2;
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rectangleTagLabel.textColor = M7TextColor;
        rectangleTagLabel.backgroundColor = [UIColor clearColor];
        rectangleTagLabel.text = self.hotSearches[i];
        rectangleTagLabel.py_width = contentView.py_width / M7RectangleTagMaxCol;
        ...
        rectangleTagLabel.py_x = rectangleTagLabel.py_width * (i % M7RectangleTagMaxCol);
        rectangleTagLabel.py_y = rectangleTagLabel.py_height * (i / M7RectangleTagMaxCol);
    ...
    self.hotSearchView.py_height = CGRectGetMaxY(contentView.frame) + M7SEARCH_MARGIN * 2;
    ...
    for (int i = 0; i < M7RectangleTagMaxCol - 1; i++) {
        ...
        verticalLine.py_x = contentView.py_width / M7RectangleTagMaxCol * (i + 1);
        verticalLine.py_width = 0.5;
        [contentView addSubview:verticalLine];
    }
    
    for (int i = 0; i < ceil(((double)self.hotSearches.count / M7RectangleTagMaxCol)) - 1; i++) {
...
- (void)setupHotSearchRankTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rankView.py_width = (self.baseSearchTableView.py_width - M7SEARCH_MARGIN * 3) * 0.5;
        ...
        [rankTag sizeToFit];
        rankTag.py_width = rankTag.py_height += M7SEARCH_MARGIN * 0.5;
        ...
        rankTextLabel.textColor = M7TextColor;
        rankTextLabel.font = [UIFont systemFontOfSize:14];
        rankTextLabel.py_x = CGRectGetMaxX(rankTag.frame) + M7SEARCH_MARGIN;
        rankTextLabel.py_width = (self.baseSearchTableView.py_width - M7SEARCH_MARGIN * 3) * 0.5 - rankTextLabel.py_x;
    ...
    for (int i = 0; i < self.rankViews.count; i++) { // default is two column
        ...
        rankView.py_x = (M7SEARCH_MARGIN + rankView.py_width) * (i % 2);
...
- (void)setupSearchHistoryTags
{
    ...
    self.searchHistoryTagsContentView.py_y = M7SEARCH_MARGIN;
    self.emptyButton.py_y = self.searchHistoryHeader.py_y - M7SEARCH_MARGIN * 0.5;
    self.searchHistoryTagsContentView.py_y = CGRectGetMaxY(self.emptyButton.frame) + M7SEARCH_MARGIN;
...
- (NSArray *)addAndLayoutTagsWithTagsContentView:(UIView *)contentView tagTexts:(NSArray<NSString *> *)tagTexts;
{
    ...
    for (int i = 0; i < contentView.subviews.count; i++) {
        ...
        if (currentX + subView.py_width + M7SEARCH_MARGIN * countRow > contentView.py_width) {
            subView.py_x = 0;
            subView.py_y = (currentY += subView.py_height) + M7SEARCH_MARGIN * ++countCol;
            currentX = subView.py_width;
            countRow = 1;
        } else {
            subView.py_x = (currentX += subView.py_width) - subView.py_width + M7SEARCH_MARGIN * countRow;
            subView.py_y = currentY + M7SEARCH_MARGIN * countCol;
            countRow ++;
        }
    }
    ...
    if (self.hotSearchTagsContentView == contentView) { // popular search tag
        ...
        self.searchHistoryView.py_height = CGRectGetMaxY(contentView.frame) + M7SEARCH_MARGIN * 2;
    }
...
- (void)layoutForDemand {
    if (NO == self.swapHotSeachWithSearchHistory) {
        self.hotSearchView.py_y = M7SEARCH_MARGIN * 2;
        self.searchHistoryView.py_y = self.hotSearches.count > 0 && self.showHotSearch ? CGRectGetMaxY(self.hotSearchView.frame) : M7SEARCH_MARGIN * 1.5;
    } else { // swap popular search whith search history
        self.searchHistoryView.py_y = M7SEARCH_MARGIN * 1.5;
        self.hotSearchView.py_y = self.searchHistories.count > 0 && self.showSearchHistory ? CGRectGetMaxY(self.searchHistoryView.frame) : M7SEARCH_MARGIN * 2;
    }
}
...
- (void)setSearchHistoryTitle:(NSString *)searchHistoryTitle
{
    ...
    if (M7SearchHistoryStyleCell == self.searchHistoryStyle) {
...
- (void)setHotSearches:(NSArray *)hotSearches
{
    ...
    if (0 == hotSearches.count || !self.showHotSearch) {
        ...
        if (M7SearchHistoryStyleCell == self.searchHistoryStyle) {
            ...
            tableHeaderView.py_height = M7SEARCH_MARGIN * 1.5;
    ...
    if (M7HotSearchStyleDefault == self.hotSearchStyle
        || M7HotSearchStyleColorfulTag == self.hotSearchStyle
        || M7HotSearchStyleBorderTag == self.hotSearchStyle
        || M7HotSearchStyleARCBorderTag == self.hotSearchStyle) {
        [self setupHotSearchNormalTags];
    } else if (M7HotSearchStyleRankTag == self.hotSearchStyle) {
        [self setupHotSearchRankTags];
    } else if (M7HotSearchStyleRectangleTag == self.hotSearchStyle) {
        [self setupHotSearchRectangleTags];
    }
    [self setSearchHistoryStyle:self.searchHistoryStyle];
}

- (void)setSearchHistoryStyle:(M7SearchHistoryStyle)searchHistoryStyle
{
    ...
    switch (searchHistoryStyle) {
        case M7SearchHistoryStyleColorfulTag:
            for (UILabel *tag in self.searchHistoryTags) {
                tag.textColor = [UIColor whiteColor];
                tag.layer.borderColor = nil;
                tag.layer.borderWidth = 0.0;
                tag.backgroundColor = M7SEARCH_COLORPolRandomColor;
            }
            break;
        case M7SearchHistoryStyleBorderTag:
            for (UILabel *tag in self.searchHistoryTags) {
                tag.backgroundColor = [UIColor clearColor];
                tag.layer.borderColor = M7SEARCH_COLOR(223, 223, 223).CGColor;
                tag.layer.borderWidth = 0.5;
            }
            break;
        case M7SearchHistoryStyleARCBorderTag:
            for (UILabel *tag in self.searchHistoryTags) {
                tag.backgroundColor = [UIColor clearColor];
                tag.layer.borderColor = M7SEARCH_COLOR(223, 223, 223).CGColor;
                tag.layer.borderWidth = 0.5;
...
- (void)setHotSearchStyle:(M7HotSearchStyle)hotSearchStyle
{
    _hotSearchStyle = hotSearchStyle;
    
    switch (hotSearchStyle) {
        case M7HotSearchStyleColorfulTag:
            ...
            break;
        case M7HotSearchStyleBorderTag:
            for (UILabel *tag in self.hotSearchTags) {
                tag.backgroundColor = [UIColor clearColor];
                tag.layer.borderColor = M7SEARCH_COLOR(223, 223, 223).CGColor;
                tag.layer.borderWidth = 0.5;
            ...
            break;
        case M7HotSearchStyleARCBorderTag:
            for (UILabel *tag in self.hotSearchTags) {
                tag.backgroundColor = [UIColor clearColor];
                tag.layer.borderColor = M7SEARCH_COLOR(223, 223, 223).CGColor;
                tag.layer.borderWidth = 0.5;
            ...
            break;
        case M7HotSearchStyleRectangleTag:
            self.hotSearches = self.hotSearches;
            break;
        case M7HotSearchStyleRankTag:
...
- (void)setSearchViewControllerShowMode:(M7SearchViewControllerShowMode)searchViewControllerShowMode
{
    _searchViewControllerShowMode = searchViewControllerShowMode;
    if (_searchViewControllerShowMode == M7SearchViewControllerShowModeModal) { // modal
        self.navigationItem.hidesBackButton = YES;
        self.navigationItem.rightBarButtonItem = _cancelBarButtonItem;
        self.navigationItem.leftBarButtonItem = nil;
    } else if (_searchViewControllerShowMode == M7SearchViewControllerShowModePush) { // push
        self.navigationItem.hidesBackButton = YES;
...
- (void)emptySearchHistoryDidClick
{
    ...
    M7SEARCH_LOG(@"%@", [NSBundle py_localizedStringForKey:M7SearchEmptySearchHistoryLogText]);
}
...
- (void)tagDidCLick:(UITapGestureRecognizer *)gr
{
    ...
    M7SEARCH_LOG(@"Search %@", label.text);
}
...
- (void)handleSearchResultShow
{
    switch (self.searchResultShowMode) {
        case M7SearchResultShowModePush:
            ...
            break;
        case M7SearchResultShowModeEmbed:
            if (self.searchResultController) {
                ...
                M7SEARCH_LOG(@"M7SearchDebug： searchResultController cannot be nil when searchResultShowMode is M7SearchResultShowModeEmbed.");
            }
            break;
        case M7SearchResultShowModeCustom:
...
#pragma mark - M7SearchSuggestionViewDataSource
...
{
    if (M7SearchResultShowModeEmbed == self.searchResultShowMode && self.showSearchResultWhenSearchTextChanged) {
        [self handleSearchResultShow];
...
{
    if (M7SearchResultShowModeEmbed == self.searchResultShowMode) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return self.showSearchHistory && M7SearchHistoryStyleCell == self.searchHistoryStyle ? self.searchHistories.count : 0;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *cellID = @"M7SearchHistoryCellID";
    
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
        cell.textLabel.textColor = M7TextColor;
...
{
    return self.showSearchHistory && self.searchHistories.count && M7SearchHistoryStyleCell == self.searchHistoryStyle ? (self.searchHistoryTitle.length ? self.searchHistoryTitle : [NSBundle py_localizedStringForKey:M7SearchSearchHistoryText]) : nil;
}

- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    return self.searchHistories.count && self.showSearchHistory && M7SearchHistoryStyleCell == self.searchHistoryStyle ? 25 : 0.01;
}
